...
def main():
    ...
    for step in range(steps):
        ...
        T = max(0.000001, ((steps - step)/steps)**3-.005) 
        ...
        for i in range(tracks):
            ...
            if T==0:                                              # If T = 0, we use hill climbing algorithm
                if S_new > S_old:
                    x[i], y[i] = x_new, y_new                     # new solution is better, go there 
            elif random.random() < np.exp(-(S_old-S_new)/T):      # If T!=0, we use simulated annealing
                    x[i], y[i] = x_new, y_new                     # new solution is better, go there 
            else:
